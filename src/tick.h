...
//
//   Parser → Analysis → Codegen → C code
//
...
// ----------------------------------------------------------------------------
// The analysis pass performs type resolution, type checking, and AST
// transformations to prepare for codegen.
//
// MUST ESTABLISH:
...
//
// ANALYSIS-GENERATED CONSTRUCTS:
// The analysis pass may introduce TICK_AST_GOTO_STMT and TICK_AST_LABEL_STMT
// nodes for control flow transformations (defer, errdefer, error handling).
// These nodes are NEVER generated by the parser.
//
// MAY PRESERVE:
...
// Arithmetic operations are NOT transformed during analysis. Codegen handles
// them by mapping (operation, resolved_type) to runtime functions
//
// ----------------------------------------------------------------------------
...
// Analysis is responsible for inserting forward declarations and ordering
// decls.
//
// REQUIRES:
...
//    temporaries by analysis
//    - Example transformation by analysis:
//      * Input:  Point { x: foo() + 1, y: 2 }
//      * Analysis creates: let __tmp1: i32 = foo() + 1;
//                          Point { x: __tmp1, y: 2 }
...
//    expressions
//
// ALLOWED ANALYSIS-GENERATED NODES:
// Codegen DOES accept TICK_AST_GOTO_STMT and TICK_AST_LABEL_STMT nodes,
// which are introduced by the analysis pass for control flow transformations.
//
// CODEGEN BEHAVIOR:
...
// - Unsupported high-level type → Fatal error (analysis didn't transform)
...
// - Union with tag_type=NULL → Fatal error (analysis didn't generate tag)
//
// ----------------------------------------------------------------------------
// TYPE DECLARATION REQUIREMENTS
// ----------------------------------------------------------------------------
// For struct/enum/union declarations to work correctly, analysis must
...
//    - After analysis: all UNION_DECL nodes have non-NULL tag_type
//
// CODEGEN CAN ASSUME:
